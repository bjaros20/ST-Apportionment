...
for (state_name in names(result_list)) {
  ...
  current_df <- na.omit(current_df[, c("State_Acronym", "year", "log_ci", "Post")])
  ...
  current_sDiD <- panel.matrices(current_df, unit = "State_Acronym", time = "year", outcome = "log_ci", treatment = "Post")
...
sDiD_log_ci_list_CI_long_run <- data.frame(
  State = character(),
...
# p_value_right_tail <-pt(t_statistic, df = nrow(current_df) - 1, lower.tail = FALSE)
  ...
  sDiD_log_ci_list_CI_long_run <- rbind(
    sDiD_log_ci_list_CI_long_run,
    data.frame(
      State = state_name,
...
#Loop runs the SHORT PE for log_ci.  It includes the Summary Statistics.
...
for (state_name in names(result_list)) {
  ...
  # Drop states that have NA for ratio (like Alaska because no Income Tax)
  current_df <- na.omit(current_df[, c("State_Acronym", "State_Name", "year", "log_ci", "Post")])
  ...
  treatment_stats <- current_df %>%
    ...
    summarise(
      ...
      Mean = mean(log_ci, na.rm = TRUE),
      Standard_Deviation = sd(log_ci, na.rm = TRUE),
      Median = median(log_ci, na.rm = TRUE),
      Minimum = min(log_ci, na.rm = TRUE),
      Maximum = max(log_ci, na.rm = TRUE),
      IQR_25th = quantile(log_ci, 0.25, na.rm = TRUE),
      IQR_75th = quantile(log_ci, 0.75, na.rm = TRUE)
    )
...
#loop for log_ci in the Long Run. Includes the summary statistics.
# Loop over each dataframe in result_list_long_run
for (state_name in names(result_list_long_run)) {
  # Access the dataframe
  current_df <- result_list_long_run[[state_name]]
