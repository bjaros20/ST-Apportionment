# Percentage Change in Non Corporate Tax Revenue
#Load Packages
library(tidyr)
library(dplyr)
library(tidyverse)
...
Controls <- read.csv("sDiD_point_estimate_list_NCI_long_run.csv")
...
#PART I- Filter for initial dataframes
...
for (state_name in names(result_list_controls)) {
  ...
  synthetic_df <- treated_df %>%
    mutate(weighted_ci = real_NCI_cap * Weight)
...
write.csv(sum_weight_df,"NCI_sDiD_weights_SUMMED_by_state.csv", row.names = FALSE)

CI_sum_weight <- read.csv("sDiD_weights_SUMMED_by_state.csv")
...
for (state_name in names(result_list_syn_state)) {
  ...
  base76_state <- syn_state %>%
    ...
    mutate(
      real_NCI_cap_76 = (real_NCI_cap / real_NCI_cap[year == 1976]) * 100,
  ...
  diff_base76 <- base76_state %>%
    group_by(State_Acronym) %>%
    mutate(diff = abs(real_NCI_cap_76 - syn_state_76)) %>%
    ungroup()
  ...
  objective_function <- function(shift) {
    ...
    difference <- abs(filtered_data$real_NCI_cap_76 - (filtered_data$syn_state_76 + shift))
  ...
  optimal_shifts_df <- rbind(optimal_shifts_df, data.frame(State_Name = gsub("_synthetic_NCI", "", state_name), Optimal_Shift = optimal_shift))
  ...
  csv_filename <- paste0(gsub("_synthetic_NCI", "", state_name), "_base76_shift.csv")
  write.csv(shift_base_76, csv_filename, row.names = FALSE)
  
  # Save the shifted dataframe in the results list for further use
  results_percentage[[paste0(state_name, "_shift_NCI")]] <- shift_base_76
}

write.csv(optimal_shifts_df,"NCI_optimal_shift_by_state.csv", row.names = FALSE)

CI_shift <- read.csv(optimal_shift_by_state.csv)
...
for (state_name in names(results_percentage)) {
  ...
  clean_state_name <- sub("_synthetic_shift_NCI", "", state_name)
  ...
  plot1 <- ggplot(syn_state, aes(x = year)) +
    ...
    geom_line(aes(y = real_NCI_cap, color = "Actual"), size = 1.2) +
    geom_line(aes(y = syn_state, color = "Synthetic"), linetype = "dotdash", size = 1.2) +
    labs(title = paste("Actual vs. Synthetic Non Corporate Tax Revenue per Capita -", clean_state_name),
         x = "Year",
         y = "Real NC Tax Revenue per Capita ($)") +
    ...
    guides(color = guide_legend(title = "Tax Revenue Type")) +
    theme_stata()
  
  # Save plot 1
  ggsave(filename = paste0(clean_state_name, "_synthetic_vs_actual_NCI.png"), plot = plot1)
  
  # Base Year 1976 synthetic vs Actual plot
  plot2 <- ggplot(syn_state, aes(x = year)) +
    geom_vline(xintercept = year_effective_first, color = "black", linewidth = .75) +
    geom_line(aes(y = real_NCI_cap_76, color = "Actual"), size = 1.2) +
    geom_line(aes(y = syn_state_76, color = "Synthetic"), linetype = "dotdash", size = 1.2) +
    labs(title = paste("Actual vs. Syn NC Tax Revenue per Capita", clean_state_name, "- Base Year 1976"),
         x = "Year",
         y = "Real NC Tax Revenue per Capita Normalized to 1976") +
    scale_color_manual(values = c("Actual" = "red", "Synthetic" = "blue"),
                       labels = c("Actual" = paste("Real", clean_state_name), "Synthetic" = paste("Synthetic", clean_state_name))) +
    guides(color = guide_legend(title = "Non Corporate Tax Revenue Type")) +
    theme_stata()
  
  # Save plot 2
  ggsave(filename = paste0(clean_state_name, "_base_1976_synthetic_vs_actual_NCI.png"), plot = plot2)
  ...
  plot3 <- ggplot(syn_state, aes(x = year)) +
    ...
    labs(title = paste("Actual vs. Syn NC Tax Revenue per Capita", clean_state_name, "- Base 1976 & Shift"),
         x = "Year",
         y = "Real NC Tax Revenue per Capita Normalized to 1976") +
    scale_color_manual(values = c("Actual" = "red", "Synthetic" = "blue"),
  ...
  # Save plot 3
  ggsave(filename = paste0(state_name, "_shift76_synthetic_vs_actual_NCI.png"), plot = plot3)
}
